...
// --- Helper to set version object from a version string like "9.00" ---
function setVersionFromString(versionStr) {
    ...
    version.minor = parseInt(parts[1], 16); // minor is given as hex (e.g., "02" -> 2)
    version.console = 4; // assume PS4 (only PS4 is supported)
}

// --- Modified doJb with optional version string ---
async function doJb(versionStr) {
    // If versionStr is provided, use it; otherwise detect from UA
    if (versionStr) {
        setVersionFromString(versionStr);
    } else {
        version.init(); // detects from navigator.userAgent
    }
    ...
    try {
        // (version is now set)
        // The rest of the exploit loading is unchanged
        ...
        if (fn.setuid.invoke(0) === -1) {
            ...
            const payloadFile = `payload_${major}${minor}.bin`; // e.g., payload_900.bin, payload_1102.bin
            let bin_rsp;
            try {
                bin_rsp = await fetch(`src/${payloadFile}`);
                if (!bin_rsp.ok) throw new Error('Payload not found');
            } catch {
                // fallback to default payload.bin
